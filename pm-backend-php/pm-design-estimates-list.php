...
$all = !empty($_GET['all']);
$sql = "SELECT estimate_id, deliverable, complexity, definition, unit,
               frd_best, frd_worst, nofrd_best, nofrd_worst, sort_order, is_active
        FROM design_estimates" . ($all ? "" : " WHERE is_active = 1") . "
...
echo json_encode(['estimates' => $rows, 'hours_per_day' => DESIGN_HOURS_PER_DAY]);
