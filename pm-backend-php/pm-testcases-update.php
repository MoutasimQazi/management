...
$check = $pdo->prepare("SELECT case_id, title, steps, expected FROM test_cases WHERE case_id = ? AND $scope");
$check->execute(array_merge([$caseId], $params));
$existing = $check->fetch();
if (!$existing) denyNotYours();

$stmt = $pdo->prepare("UPDATE test_cases SET title = ?, steps = ?, expected = ? WHERE case_id = ?");
$stmt->execute([
