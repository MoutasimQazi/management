  ...
  try {
    ...
    el.innerHTML =
      ...
      '<form id="newTaskForm" class="inline-form">' +
        ...
        '</div>' +
        '<div class="row">' +
          '<div class="field"><label>ETA <span class="opt">— optional</span></label><input id="taskEta" type="date" /></div>' +
    ...
    document.getElementById('newTaskForm').addEventListener('submit', async (e) => {
      ...
      const eta = document.getElementById('taskEta').value || null;
      const priority = document.getElementById('taskPriority').value;
      const description = document.getElementById('taskDescription').value.trim();
      if (!task_name || !employee_id) return;
      try {
        await api(PM_TASKS_CREATE_URL, { method: 'POST',
          body: { project_id: projectId, employee_id: Number(employee_id), task_name, description, eta, priority } });
        e.target.reset();
        e.target.classList.remove('open');
        renderProjectDetail(projectId);
      } catch (err) {
  ...
  return '<div class="task-row" data-task-row="' + t.task_id + '">' +
    '<div class="tinfo"><div class="ttitle">' + esc(t.task_name) + '</div>' +
      '<div class="tmeta">' +
        ...
        (t.eta ? '<span>ETA ' + esc(fmtDay(t.eta)) + '</span>' : '') +
...
function taskEditForm(t){
  ...
  return '<div class="task-row edit-row" data-task-row="' + t.task_id + '">' +
    '<div class="row" style="flex:1">' +
      ...
      '<div class="field"><input type="date" value="' + esc(t.eta ? String(t.eta).slice(0,10) : '') + '" data-task-field="eta" /></div>' +
...
function wireTaskRows(root, onChanged){
  ...
  root.querySelectorAll('[data-task-save]').forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      const eta = rowEl.querySelector('[data-task-field="eta"]').value || null;
      ...
      try {
        await api(PM_TASKS_UPDATE_URL, { method: 'POST',
          body: { task_id: id, task_name, employee_id: Number(employee_id), eta, priority, description } });
        onChanged();
      } catch (err) {
...
function questionCard(q){
  ...
  return '<div class="question-card">' +
    '<div class="qmeta">' + badge(q.status) + taskRef + '<span>' + esc(fmtDate(q.created_at)) + '</span></div>' +
